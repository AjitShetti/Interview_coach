...
async def generate_report(
    ...
    """Generate a report for a completed session and persist it."""
    ...
    # Rebuild coach from transcript if not in memory
    ...
    if session_id_str not in _active_coaches:
        ...
        # Reconstruct session state from DB transcript
        from interview_coach import InterviewSession as CoachSession, InterviewPhase
    ...
    # Generate report via InterviewCoach
...
async def get_report(
    ...
    """Fetch a saved report, verifying session ownership."""
...
async def list_reports(
    ...
    """List all reports for sessions belonging to the user."""
    result = await db.execute(
        select(Report)
